...
terraclimate_SERVER = '/home/caro/OGGM_terraclimate/'


def set_terraclimate_url(url):
    """If you want to use a different server for TerraClimate (for testing, etc)."""
    global terraclimate_SERVER
    terraclimate_SERVER = url


@utils.locked_func
def get_terraclimate_file(var=None):
    """Returns a path to the desired TerraClimate baseline climate file.
    ...
    if var not in ['tmp', 'pre']:
        raise InvalidParamsError('terraclimate variable {} '
                                 'does not exist!'.format(var))

    # File to look for
    if var == 'tmp':
        bname = 'TC_tmean_hgt_2022.nc'
    else:
        bname = 'TC_pr_2022.nc'
                 
    h_url = terraclimate_SERVER + bname #+ '.bz2'
    #return utils.file_extractor(utils.file_downloader(h_url))
    return h_url


@entity_task(log, writes=['climate_historical'])
def process_terraclimate_data(gdir, y0=None, y1=None, output_filesuffix=None):
    """Processes and writes the TerraClimate baseline climate data for this glacier.
    ...
    if cfg.PARAMS['baseline_climate'] != 'terraclimate': # change HISTALP por TerraClimate
        raise InvalidParamsError("cfg.PARAMS['baseline_climate'] should be "
                                 "set to terraclimate.")


    # read the time out of the pure netcdf file
    ft = get_terraclimate_file('tmp')
    fp = get_terraclimate_file('pre')
    
    #import netCDF4 as nc
    # editar fecha de inicio en nc codigo R
    with utils.ncDataset(ft) as nc:
        vt = nc.variables['time']#.Dataset(ft)
      #  assert vt[0] == 1
      #  assert vt[-1] == vt.shape[0]
        t0 = vt.units.split(' since ')[1][:7] # comienzo '1958'
